...
class Maze(object):
    ...
    def turtle_registration(self):
        ...
        turtle.register_shape('tri', ((-4, -3), (0, 4), (4, -3), (0, 0)))
    ...
    def fix_wall_inconsistency(self, verbose = True):
        ...
        '''
        wall_erors = self.check_wall_inconsistency()

        if wall_erors and verbose:
            print('Warning: maze contains wall inconsistency.')

        for (i,j), error in wall_erors:
            if error == 'v':
                self.maze[i,j] |= 2
                self.maze[i,j+1] |= 8
            elif error == 'h':
    ...
    def wgt_to_color(self, wgt):

        return '#%02x00%02x' % (int(wgt * 255), int((1 - wgt) * 255))
    ...
    def show_particles(self, particles, show_frequency = 10):
        ...
        for i, particle in enumerate(particles):
            if i % show_frequency == 0:
                ...
                turtle.color(self.wgt_to_color(particle.wgt))
                turtle.stamp()
    ...
    def show_estimated_location(self, particles):
        '''
        Show average wgted mean location of the particles.
        '''
        ...
        for particle in particles:
            ...
            wgt_accum += particle.wgt
            x_accum += particle.x * particle.wgt
            y_accum += particle.y * particle.wgt
            heading_accum += particle.heading * particle.wgt
        ...
        heading_estimate = heading_accum / wgt_accum
...
class Particle(object):
    ...
    def __init__(self, x, y, maze, heading = None, wgt = 1.0, sensor_lmt = None, noisy = False):
    ...
    def read_sensor(self, maze):
        ...
        if self.sensor_lmt is not None:
            for i in range(len(readings)):
                if readings[i] > self.sensor_lmt:
                    readings[i] = self.sensor_lmt
...
class Robot(Particle):
    ...
    def __init__(self, x, y, maze, heading = None, speed = 1.0, sensor_lmt = None, noisy = True):

        super(Robot, self).__init__(x = x, y = y, maze = maze, heading = heading, sensor_lmt = sensor_lmt, noisy = noisy)
        self.step_count = 0
        self.noisy = noisy
        self.time_step = 0
        self.speed = speed
...
class wgtedDistribution(object):
    ...
    def random_select(self):
        ...
        try:
            ...
            # When all particles have wgts zero
            return None
...
def wgt_gaussian_kernel(x1, x2, std = 10):
